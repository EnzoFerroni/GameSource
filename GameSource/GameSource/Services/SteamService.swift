...
class SteamService {
    ...
    func fetchUserGames(steamId: String) async throws -> [SteamGame] {
        ...
        let response = try JSONDecoder().decode(SteamGamesResponse.self, from: data)
        
        return response.response.games.sorted { $0.playtimeForever > $1.playtimeForever }
    }
    
    func fetchUserProfile(steamId: String) async throws -> SteamUserProfile {
